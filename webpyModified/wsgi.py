...
def runwsgi(func):
    ...
    if 'numthreads' in sys.argv:
        args = sys.argv[1:]
        args.remove('numthreads')
        # runsimpleOfNumthread(func, server_address=("172.0.0.1", 8080), numthreads=10,
        #         max=-1, request_queue_size=5):
        return httpserver.runsimpleOfNumthread(func, server_address=validip(listget(args, 0, '')),
            numthreads=int(listget(args, 1, 10)), max=int(listget(args, 2, -1)), request_queue_size=int(listget(args, 3, 5)))
