...
def runsimpleOfNumthread(func, server_address=("172.0.0.1", 8080), numthreads=10,
    ...
    server = WSGIServerOfNumthreads(server_address, func, numthreads,
            max, request_queue_size)
...
def WSGIServerOfNumthreads(server_address, wsgi_app, numthreads, 
            max, request_queue_size):
    ...
    server = wsgiserver.CherryPyWSGIServer(server_address, wsgi_app, numthreads=numthreads, 
        server_name="localhost", max=max, request_queue_size=request_queue_size)
